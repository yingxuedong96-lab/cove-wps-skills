/**
 * copy-from-linked.js - 样式复制（智能模式）
 * 版本: 26.0410.1012
 * 参数:
 *   - sourceDocPath: 关联文档路径（可选）
 * 模式：
 *   - 有 sourceDocPath → 从关联文档复制样式到当前文档
 *   - 无 sourceDocPath → 从当前文档提取样式
 */
try {
  var VER = "26.0410.1012";
  ...
  if (sourcePath) {
    ...
    console.log("[copy-style] 目标文档: " + DOC.Name);
    ...
    console.log("[copy-style] 提取样式数: " + Object.keys(styles).length);

    // 关闭源文档
    try {
      sourceDoc.Close(false);
      console.log("[copy-style] 已关闭源文档");
    } catch (e) {}
    ...
    console.log("[copy-style] 文档: " + DOC.Name);

    var styles = extractStyles(DOC);
    var styleCount = Object.keys(styles).length;
    console.log("[copy-style] 提取样式数: " + styleCount);

    // 保存到 latest-template.json
    var templateDir = "/Users/cassia/Desktop/dyx/wpsjs/模版生成/";
    var latestFile = templateDir + "latest-template.json";
    var jsonContent = JSON.stringify({
      version: VER,
      docName: DOC.Name,
      docType: styles.isPaper ? "论文报告" : "公文",
      styles: styles,
    ...
    try {
      ...
      jsonDoc.SaveAs2(latestFile, 7);
      jsonDoc.Close(false);
      saved = true;
      console.log("[copy-style] 模板已保存");
    } catch (e) {
      console.log("[copy-style] 保存失败: " + String(e));
    }
    ...
    var styleNames = {
      ...
      heading3: "三级标题", body: "正文", figureCaption: "图名",
      tableCaption: "表名", appendixTitle: "附录标题"
    };
    for (var t in styles) {
      if (styles[t].count > 0 && t !== 'isPaper') {
        detailLines.push((styles[t].name || styleNames[t] || t) + ": " + styles[t].count + "处");
      }
    }
  ...
  function applyStyles(doc, styles) {
    ...
    function detectType(text, isPaper, fmt) {
      if (isPaper) {
        ...
        if (/^\d+\.\d+[^.\d]/.test(text)) return "heading2";
        if (/^\d+\s+[^\d\.\s]/.test(text)) return "heading1";
        if (/^附\s*录\s*[A-Z]/.test(text)) { lastAppendixTitle = true; return "appendixTitle"; }
        if (/^[A-Z]\.\d+/.test(text)) return "appendixSection";
        if (/^图\s*\d+/.test(text)) return "figureCaption";
        if (/^表\s*\d+/.test(text)) return "tableCaption";
        if (/^[\(（]\d+[\)）]/.test(text)) return "listItem";
    ...
    for (var t in appliedCounts) {
      if (appliedCounts[t] > 0) {
        ...
        detailLines.push((styles[t] ? styles[t].name : styleNames[t] || t) + ": " + appliedCounts[t] + "处");
      }
    }
