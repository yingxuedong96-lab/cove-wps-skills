/**
 * extract-template.js - 完整样式提取（显示元素名称和示例）
 * 版本: 26.0409.1030
 */
try {
  var VER = "26.0409.1030";
  ...
  function detectStyle(text, isPaper) {
    ...
    if (isPaper) {
      ...
      if (/^\d+[^.\d\s]/.test(text)) return "heading1";
    ...
    // 列表项
    if (/^[a-z][\)）\.]/.test(text) || /^[\(（][a-z][\)）]/.test(text)) return "listItem";
  ...
  for (var i = 1; i <= paras.Count; i++) {
    ...
    // 智能推断
    if (!type) {
      if (fmt.bold && fmt.fontSize >= 18) type = isPaper ? "heading1" : "docTitle";
  ...
  sortedTypes.forEach(function(t) {
    ...
    if (fmt.alignment !== undefined && alignMap[fmt.alignment]) {
      parts.push("对齐: " + alignMap[fmt.alignment]);
    }
    if (parseFloat(fmt.firstLineIndent) > 0) {
      parts.push("首行缩进: " + fmt.firstLineIndent + "字符");
    }
    if (parseFloat(fmt.leftIndent) > 0) {
      parts.push("左缩进: " + fmt.leftIndent + "字符");
    }
    if (fmt.lineSpacing > 0) {
      parts.push("行距: " + (fmt.lineSpacing / 20).toFixed(1) + "pt");
    }
